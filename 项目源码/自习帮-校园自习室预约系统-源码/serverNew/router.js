...
// 精准用户信息
router.get('/oneUser', async (ctx) => {
    let userInfo = await student.getOneUser(ctx.query);
    ctx.body = {
        sucess: true,
        data: userInfo
    };
...
router.get('/getRoomLists', async (ctx, next) => {
    ...
    // console.log("ctxxxxxxxxxxxxxxxxx");
...
// 获取自己在自习室的座位
router.get('/getMySeatIndex', async (ctx) => {
    let callback = await room.getMySeatIndex(ctx.query)
    ctx.body = callback;
})
...
// 删除加入的自习室
router.get('/deleteHasRoom', async (ctx) => {
...
// 获取已预约的自习室列表
router.get('/getHasRoomList', async (ctx) => {
    let callback = await student.getHasRoomList(ctx.query);
    ctx.body = callback;
})
// 获取待审核的自习室列表
router.get('/getReviewRoomList', async (ctx) => {
    let callback = await student.getReviewRoomList(ctx.query);
    ctx.body = callback;
})

// 获取已收藏的自习室列表
router.get('/getCollectRoomList', async (ctx) => {
    let callback = await student.getCollectRoomList(ctx.query);
    ctx.body = callback;
})
